...
3. Todos os campos do kickoff com códigos e traduções (versão atual do NEXUS)
...
Sempre seguir a ordem definida em `_ROUTER.json` → `ordem_capitulos`. Sequência das 9 fases:

| Fase | Tópicos |
|------|---------|
| 1. Dados do Projeto | Objetivo (fixo), Visão Geral, cubagem, integracao-pedidos |
...
| 8. Integração | integracao-wcs-wms, integracao-wms-erp, integracao-cadastros |
...
srv_invent=Invent · srv_client=Cliente · srv_shared=Compartilhada · srv_tbd=A definir
...
| **Por linha PBL:** | | |
| `p_r` | select | `wcs`=WCS · `wms_only`=WMS · `both_wms_wcs`=Ambos · `tbd` |
| `p_m` | select | `led_mode`=LED · `bar_mode`=Cód. barras · `tbd` |
| `p_led` | select (se p_m=led) | `led3`=LED 3 dig · `led10`=LED 10 dig · `tbd` |
| `p_g` | select | `both_turn`=Todos os giros · `high_turn`=Alto giro · `med_turn`=Médio giro · `low_turn`=Baixo giro · `tbd` |
| `p2` | texto | Número de estações |
| `p_pdv` | select | `yes`=Tem PDV (Ponto de Desvio) · `no` |
| `p_pdv_f` | select (se p_pdv=yes) | `invent`=Invent · `client`=Cliente · `both_resp`=Ambos |
| `p_sct` | select | `sc_hand`=Scanner de mão · `sc_fixed`=Scanner fixo · `tbd` |
| `p_sc_f` | select | `invent`=Invent · `client`=Cliente · `both_resp`=Ambos |
...
| `p10lt` | select (se p10l=yes) | `led3`=LED 3 dig · `led10`=LED 10 dig · `tbd` |
| `p_re` | select | `invent`=Invent · `client`=Cliente · `both_resp`=Ambos |
...
| `if_resp_infra_cli` | textarea (se srv_shared) | Responsabilidades do cliente |
| `if_resp_infra_inv` | textarea (se srv_shared) | Responsabilidades da Invent |
| `if_resp_srv` | select | `srv_invent` · `srv_client` · `srv_shared` · `tbd` |
...
| `if_focal` | texto | Focal TI do cliente (nome + email) |
| `if_data_infra` | texto | Data entrega da infra (DD/MM/AAAA) |
...
O NEXUS exporta o kickoff em dois formatos:

**JSON** (preferido para o ED):
```json
{
  "g1": "Logística XYZ",
  "g2": "I26.042",
...
**Multi-select** (campos `multi`): valores separados por `|||`. Para verificar se um valor está ativo:
- `cf_t1` contém `conf_blind` → verificar se `"conf_blind"` aparece na string separada por `|||`

---

*Documento atualizado em 2026-06-25 com base no KickoffPage.tsx atual do NEXUS. Versão anterior (super_md + json-docs antigo) substituída por esta estrutura modular.*
